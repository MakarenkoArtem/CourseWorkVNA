...
def _GetDefaultPaths():
    ...
    if platform == 'win32':
        ...
        libPath = join('/usr', 'lib', 'vnakit', 'libVNAKit.so')
...
class RecordingSettings:
    ...
    def __repr__(self):
        return "<VNAKitSettings: %s, rbw: %f KHz, power: %f dbm, txtr: %d, mode: %d-port>" % (self.freqRange,
                                                                                              self.rbw_khz,
                                                                                              self.outputPower_dbm,
                                                                                              self.txtr, (
                                                                                                  1 if self.mode == VNAKIT_MODE_ONE_PORT else 2))
...
def GetRecordingResult():
    ...
    resMap = {
        rxPort_i + 1: [_vnaKitComplex_2Python(c_resStruct.resultBuffer[rxPort_i][freq_i]) for freq_i in range(nFreqs)]
        for rxPort_i in range(nRxTr)}
...
class FrequencyLimits:
    ...
    def __repr__(self):
        return "<FrequencyLimits: [%f-%f] MHz, step: %f MHz, #Points: [%d-%d]>" % (self.min_MHz, self.max_MHz,
                                                                                   self.step_MHz, self.nPointsMin,
                                                                                   self.nPointsMax)
